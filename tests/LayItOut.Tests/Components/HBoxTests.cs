...
namespace LayItOut.Tests.Components
{
    public class HBoxTests
    {
        ...
        public void Measure_should_pass_size_to_the_component_with_width_decreased_by_the_width_of_previous_ones_but_not_smaller_than_0()
        {
            ...
            box.Measure(new Size(30, 100), TestRenderingContext.Instance);

            c1.Verify(x => x.Measure(new Size(30, 100), TestRenderingContext.Instance));
            c2.Verify(x => x.Measure(new Size(15, 100), TestRenderingContext.Instance));
            c3.Verify(x => x.Measure(new Size(0, 100), TestRenderingContext.Instance));
        }
